...
class Navbar {
    constructor() {
        this.obj = SELECT.one("#nav");
        this.colorschema = "dark";
        const [s1, s2] = ["section", "section"].map((e) => new TAG_HTML(e).class(["navbar-section"]).attr({ colorschema: "dark" }).obj);
        this.obj.append(s1, s2);
        this.btns = ["settings", "colorschema", "logout"].map((e) => this.init_btn(e).attr({ colorschema: "dark" }).obj);
    ...
    get_events() {
        this.btns[1].addEventListener("click", () => {
            ...
            for (const e of SELECT.all("[data-colorschema]")) {
                e.dataset.colorschema = colorschema;
            } //isDark?"dark":"light";}
        });
    }
