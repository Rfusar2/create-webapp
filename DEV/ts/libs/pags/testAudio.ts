class AudioPage {
    ...
    constructor(parent:HTMLElement){
        ...
        const [container, title ] = ["div", "h1"].map((e:string)=>new TAG_HTML(e).obj)
        container.classList.add("s2-header")
        container.append(title)
        s2.append(container)

        for(const src of this.audio){ this.createRow(s1, title, src) }

    }

    createRow(s1:HTMLElement, title:HTMLElement, src:string){
